...
int main(int argc,char *argv[]) {
   ...
   int sock;
   int pcount=0;
   long long maxpacket=0;
   if(argv[3] > 0) {
//      maxpacket=strtol(argv[3],(char **)NULL, 10);
        maxpacket=strtollong(argv[3]);
   }
...
#endif
      fprintf(stderr,"<target ip> <target port> [packet count (optional)]\n");
      exit(1);
   }
